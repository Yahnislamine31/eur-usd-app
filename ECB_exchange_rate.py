    ...
    if st.button("🚀  Generate & Download", type="primary"):
        ...
        if all_ok:
            ...
            if all_ok:
                ...
                freq_note = ""
                if fx_ready and fx_frequencies:
                    freq_note = f" The .do file also derives {', '.join(fx_frequencies).lower()} FX data from the daily series."
                st.success(
                    f"✅ Done! {n} sheet{'s' if n != 1 else ''} in the Excel "
                    f"+ Stata .do file — both packed in the zip below.{freq_note}"
                )
